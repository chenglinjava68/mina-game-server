...
public class ClientProtocolCumulativeDecoder extends CumulativeProtocolDecoder {
  ...
  protected boolean doDecode(IoSession session, IoBuffer buf, ProtocolDecoderOutput out) throws Exception {
    if(buf.remaining() >= ServerConfig.HEAD_LENGTH) {// 有数据，先读取报头
      ...
      int head_bodyLength=buf.getInt();// 报体长度
      if(buf.remaining() < head_bodyLength) {// 不完整的数据包(报体长度不完整,需拼凑完整数据)
        ...
        // 封装MyBuffer
        MyBuffer myBuf=MyBuffer.allocate(head_bodyLength);
        myBuf.put(head_ptlType);
        myBuf.putShort(head_ptlId);
        myBuf.putInt(head_bodyLength);
        myBuf.put(getBodyData(buf, head_bodyLength));
        myBuf.flip();
        // 发送至Handler处理
        out.write(myBuf);
  ...
  private byte[] getBodyData(IoBuffer buf, int bodyLength) {
    // 取报体内容
